...
fun FinsibleDateRangePicker(
    ...
    firstVisibleMonth: YearMonth = selectedRange.startDate?.let { YearMonth.from(it) } ?: YearMonth.now(),
    ...
    BaseCalendarContainer(modifier = modifier, colors = colors, sizes = sizes) {
        FinsibleCalendarNavigationHeader(
            ...
            onSelectedDateClick = {
                val targetDate = normalizedRange.startDate ?: today
                coroutineScope.launch {
                    calendarState.animateScrollToMonth(
                        clampToRange(YearMonth.from(targetDate), startMonth, endMonth),
                    )
                }
            },
        )
